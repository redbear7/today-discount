...
export function BottomNavigation() {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto h-[76px] max-w-[430px] border-t border-neutral-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur">
      <div className="grid h-full grid-cols-4">
        {items.map(({ href, label, icon: Icon }, index) => {
          ...
          return (
            ...
            >
              <Icon className="h-6 w-6" strokeWidth={active ? 2.8 : 2.4} fill={active ? "currentColor" : "none"} />
              <span>{label}</span>
            </Link>
          );
        })}
